    ...
    static createNodeObjects(
      ...
      for (const nodeId of nodesToTraverse) {
        ...
        try {
          ...
          if (/processor|input/.test(componentType)) {
            const previousNodeId = PipelineService.findPreviousNodeId(flow, nodeId);
            const inputName = previousNodeId ? getInputName(previousNodeId, componentType) : '';
            const outputName = getOutputName(node, componentId, variablesAutoNaming);
